...
void GLRenderer::DrawTemple()
{
    ...
    glPushMatrix();
    double yTrans = 4 + (0.6 * 0.5);
    glTranslatef(0.0, -0.25 - yTrans, 0.0);
    DrawBox(24, 0.5, 12);
    glTranslatef(0.0, -0.25 - 0.375, 0.0);
    DrawBox(26, 0.75, 14);
    glPopMatrix();

    yTrans = 4 + 0.5 * 0.5;
    glPushMatrix();
    glTranslatef(0.0, yTrans + 0.15, 0.0);
    DrawBox(24, 0.3, 12);
    double a = 6 / cos(piconst / 6);
    double b = a - 4;
    double lift = b * sin(piconst / 6);
    double zt = b * cos(piconst / 6);
    glTranslatef(0.0, 0.15 + 0.1, 0.0);
    glPushMatrix();
    glTranslatef(0.0, lift, 6 - zt);
